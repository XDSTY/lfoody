<template>
  <div class="back-d">
    <div class="my_kuang">
      ...
      <div class="shez_tuk">
        <a href="my_shez.html"><img src="../assets/images/shez.png"
               alt=""></a>
      </div>
      <div class="div_bx_k">
        <div class="neir_Ef">
          <div class="yverjif">
            <ul>
              <li>
                <h2><i>￥</i>{{ userInfo.amount }}</h2>
                <p>余额</p>
              </li>
              <li>
                <h2>{{ userInfo.integral }}</h2>
                <p>积分</p>
              </li>
            </ul>
          </div>
          <div class="my_tik_list fenh_ziyek yuanj_sd">
            <h3 style="text-align: left;">我的订单<a @click="jumpToListPage">全部订单></a></h3>
            <ul>
              <li v-for="(item, i) in modules"
                  :key="i"
                  :value="item">
...
export default {
  ...
  methods: {
    jumpToListPage (item) {
      this.$router.push('/orderList/' + item.status)
    }
  },
  components: {
    'footer-bar': footer
...
.yverjif ul li:last-child:after {
  content: "";
  width: 1px;
  height: 0.5rem;
  background: #eee;
